...
DIMENSION = int(input())
REGLAS = []
print("¿Cúantos vecinos para que nazca una célula? (introducir dos números, primer número (enter) siguiente número) - REGLA CLÁSICA : 3 3 2 3")
...
INFO_TEXT2 = FONT_START.render("REGLAS DEL JUEGO: " +"B"+str(REGLAS[0])+str(REGLAS[1])+"/S"+str(REGLAS[2])+str(REGLAS[3]), True, WHITE)
...
GLIDERGUN_TEXT = FONT_START.render("Tecla 9: Glider Gun", True, WHITE)
PULSAR_TEXT = FONT_START.render("Tecla 0: Pulsar", True, WHITE)
REPLICATOR_TEXT = FONT_START.render("Tecla p: Replicator (Regla B36/S23)", True, WHITE)
...
def listener(tablero): #Está a la escucha del raton
    ...
    for event in pygame.event.get():  # usuario hace algo
        ...
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_p and PAUSE == 1:
            crearReplicator(int(DIMENSION/2),int(DIMENSION/2))
    ...
    for row in range(len(tablero_vacio)):
        for column in range(len(tablero_vacio)):
            try:
                ...
                if celula == 1:
                    if (vecinos_vivos > REGLAS[3]) or (vecinos_vivos < REGLAS[2]):
                        tablero_vacio[row][column] = 0
                    else:
                        tablero_vacio[row][column] = 1
                else:
                    if vecinos_vivos == REGLAS[0] or vecinos_vivos == REGLAS[1]:
                        tablero_vacio[row][column] = 1
            except:
                pass
...
def crearReplicator(x,y):
    tablero[x][y] = True
    tablero[x+1][y] = True
    tablero[x+2][y] = True
    tablero[x+3][y+1] = True
    tablero[x+4][y+2] = True
    tablero[x+4][y+3] = True
    tablero[x+4][y+4] = True
    tablero[x+3][y+4] = True
    tablero[x+2][y+4] = True
...
while not done:
    ...
    screen.blit(REPLICATOR_TEXT, (50 + WINDOW_CTE * DIMENSION, 260))
